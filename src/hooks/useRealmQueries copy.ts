...
export const useRealmQueries = () => {
  ...
  const saveEntity = async ({
    ...
    const postCreateRelations = [] as any[]
    ...
    try {
      realm.write(() => {
        ...
        // para agregar las relaciones de la otra entidad relacionada
        postCreateRelations?.forEach((item: any) => {
          ...
          const entityRelation =
            realm
              ...
              .filtered('id == $0 AND tenant == $1', item?.id, tenant)[0] ??
            null
          if (entityRelation) {
